...
for dataset, title, position in available_datasets:
    ...
    flatnav_hnsw_recall, flatnav_hnsw_latency = load_dataset(f"{dataset}_flatnav", json_filename, "latency_p99")
    flatnav_base_recall, flatnav_base_latency = load_dataset(f"{dataset}-base_flatnav", json_filename, "latency_p99")
...
plt.tight_layout()
plt.savefig("flatnav_comparison_p99.png", dpi=400)
print(f"Plot saved to flatnav_comparison_p99.png with {len(available_datasets)} dataset(s)")
# plt.show()
